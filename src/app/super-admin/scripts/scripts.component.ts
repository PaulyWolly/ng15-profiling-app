...
@Component({
  selector: 'app-super-admin-scripts',
  standalone: true,
  imports: [
    CommonModule,
    MatTabsModule,
      ...
      this.http.post<any>(`${environment.apiUrl}/api/admin/scripts/interactive-run/start`, { scriptName: script.name }, { withCredentials: true })
        .subscribe({
          next: (res) => {
            ...
            (dialogComponent as any).onSubmitInput = (input: string, sessionId: string) => {
              this.http.post<any>(`${environment.apiUrl}/api/admin/scripts/interactive-run/input`, { sessionId, input }, { withCredentials: true })
                .subscribe({
                  next: (resp) => {
                    (dialogComponent as any).updateOutput(resp.output, sessionId);
                  },
                  error: (err) => {
